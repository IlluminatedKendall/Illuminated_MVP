  ...
  return (
    <ul className="space-y-3">
      {transactions.map((transaction) => {
        ...
        return (
          <li key={transaction.id} className="rounded-xl border border-zinc-100 bg-white">
            <div className="grid grid-cols-1 gap-3 p-4 md:grid-cols-5 md:items-center">
              <button
                type="button"
                onClick={() =>
                  setExpandedTransactionId(isExpanded ? null : transaction.id)
                }
                className="grid w-full grid-cols-1 gap-3 text-left md:col-span-4 md:grid-cols-4 md:items-center"
              >
                <div>
                  <p className="text-xs uppercase tracking-wide text-zinc-400">Merchant</p>
                  <p className="mt-1 font-medium text-zinc-900">{merchantName}</p>
                </div>
                ...
                <div>
                  <p className="text-xs uppercase tracking-wide text-zinc-400">Total Price</p>
                  <p className="mt-1 font-medium text-zinc-800">
                ...
                <div className="flex items-center justify-between md:block md:text-right">
                  <div>
                    <p className="text-xs uppercase tracking-wide text-zinc-400">Item Count</p>
              ...
              <div className="border-t border-zinc-100 bg-zinc-50 px-4 py-4">
                <ul className="space-y-2">
                  {transaction.items.map((item) => (
                    <li
                      key={item.item_id}
                      className="grid grid-cols-1 gap-2 rounded-lg border border-zinc-200 bg-white p-3 md:grid-cols-3"
                    >
                      <div>
                        <p className="text-xs uppercase tracking-wide text-zinc-400">Item</p>
